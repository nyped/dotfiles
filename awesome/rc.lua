...
beautiful.border_focus = "#9ba2b0"
beautiful.border_normal = "#9ba2b0"
...
mytextclock = wibox.widget.textclock("%A %d %b, %r", 1)
	...
	local bat = wibox.widget {
		{
			id = "wrap_one",
			{
				id = "wrap_two",
				{
					...
					border_width  = 1,
					border_color  = beautiful.progressbar_fg,
					bar_border_width = 1,
					bar_border_color = beautiful.progressbar_fg
				},
					forced_height = 80,
					forced_width  = 8,
					direction     = 'east',
        ...
        { -- Right widgets
			awful.widget.watch(
				...
				function(widget, stdout)
                    out = stdout:gsub("[\n\r]", "")
                    widget.percentage:set_markup_silently(out.."<b>%</b>\n")
					widget.wrap_one.wrap_two.logo:set_value(tonumber(out))
                end,
				bat
			),
            awful.widget.watch(
                ...
                function(widget, stdout)
                    out = stdout:gsub("000[\n\r]", "")
                    widget:set_markup_silently(out.."<b>⁰c</b>\n")
                end,
            ...
            awful.widget.watch(
                ...
                function(widget, stdout)
                    out = stdout:gsub("[\n\r]", "")
                    widget:set_markup_silently(out.." <b>rpm</b>\n")
                end,
				wibox.widget.textbox()
            ),
            s.mylayoutbox,
...
client.connect_signal("request::titlebars", function(c)
    --- Hum...
    -- Tryna center the title
    -- https://github.com/awesomeWM/awesome/issues/2917
    local left = wibox.container.margin(
